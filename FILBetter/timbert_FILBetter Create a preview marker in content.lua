...
local previewMarkerContentLane = FILBetter.LoadConfig("previewMarkerContentLane")
			...
			if markersData[i].takeFound ~= nil then
				...
				if markersData[i].isSameItem == false then
					...
					reaper.SetTakeMarker(
						reaper.GetActiveTake(markersData[i].item),
						markersData[i].index,
						previewMarkerName,
						targetPosition
					)
...
function main()
	...
	if previewMarkerLocation == "mouse cursor" then
		...
		targetPosition = (
			reaper.BR_GetMouseCursorContext_Position() - reaper.GetMediaItemInfo_Value(item, "D_POSITION")
		)
				* takeRate
		...
		if items == nil or #items < 1 then
			if previewMarkerContentLane == "priority lane" then
				timbert.TooltipMsg(
					"FILBetter Create preview marker\nNo content at edit cursor position in priority lane",
					3
				)
			else
				timbert.TooltipMsg(
					"FILBetter Create preview marker\nNo content at edit cursor position in active lane",
					3
				)
			end
			ResetUserSelection(track, cursPos, startTime, endTime, selectedItemsStart, activeTrackLane)
		...
		for i = 1, #items do
			if
				reaper.GetCursorPosition() >= items[i].itemPosition
				and reaper.GetCursorPosition() <= items[i].itemPosition + items[i].itemLength
			then
