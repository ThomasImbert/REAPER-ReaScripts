-- @description VASC Mark and color item under mouse cursor as Uncertain
-- @author Thomas Imbert
-- @version 1.0
-- @about
--      Changes the color of the item under mouse cursor to the SWS custom color n°3 and stores extData. 
--
...
-- settings
local colorNumber = "3" -- references the SWS custom color number
local validationTag = "Uncertain"
...
local function main(colorNumber) 
	...
	if (checkValidation() > 2) then return end -- Do not color region and guide segment to Uncertain value if there is greater validation under that region already
