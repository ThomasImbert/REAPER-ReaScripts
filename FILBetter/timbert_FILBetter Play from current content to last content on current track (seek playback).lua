...
local startTime, endTime, startCurr, endCurr, playPos, startNext, endNext, curPos, curPosCurrent, curPosPrevious, curPosOrigin, isFinished, items
...
local function Seek()
	...
	if playPos >= endCurr then
		...
		reaper.GetSet_LoopTimeRange(
			...
			items[#items].itemPosition + items[#items].itemLength,
			false
		)
