...
--   # Lua Utilities
-- @changelog
--   # Incremented version of lua utilities and dependencies to ensure the use of proper case sensitive paths 1.929
--[[
...
function timbert.GetCompLanes(items, track) -- items[i].laneIndex must exist, items created with timbert.MakeItemArraySortByLane()
	...
	for i = 1, #itemsFirst do
		_, laneName = reaper.GetSetMediaTrackInfo_String(
			track,
			"P_LANENAME:" .. tostring(reaper.GetMediaItemInfo_Value(itemsFirst[i].item, "I_FIXEDLANE")),
			"laneName",
			false
		)
...
function timbert.SelectContentForPreview(items, previewMarkerName)
	...
	if #items == 1 then
		...
		for i = 1, #items do
			if
				(
					reaper.GetMediaItemInfo_Value(items[i].item, "D_POSITION")
					+ reaper.GetMediaItemInfo_Value(items[i].item, "D_LENGTH")
				) > previewPos
			then
