...
function filbGUI.UpdateFILBSettings()
	...
	if not filbCfg then
		...
		filbSettings.curSeekPlaybackRetriggCurPos = indexOf(
			FILBetter.seekPlaybackRetriggCurPos,
			filbCfg["seekPlaybackRetriggCurPos"]
		) - 1
		filbSettings.curSeekPlaybackEndCurPos = indexOf(
			FILBetter.seekPlaybackEndCurPos,
			filbCfg["seekPlaybackEndCurPos"]
		) - 1
	...
	rv, filbCfg["goToPreviousSnapTofirstContent"] =
		ImGui.Checkbox(ctx, "Snap forward to first content", filbCfg["goToPreviousSnapTofirstContent"])
	ImGui.SameLine(ctx, 0, 90)
	rv, filbCfg["goToNextSnapToLastContent"] =
		ImGui.Checkbox(ctx, "Snap back to last content", filbCfg["goToNextSnapToLastContent"])
	ImGui.SameLine(ctx)
	filbGUI.HelpMarker(
		"Changes the behaviour when the edit cursor is positioned before the first content or after the last content of the track."
	)
	rv, filbCfg["moveEditCurToStartOfContent"] =
		ImGui.Checkbox(ctx, "Move edit cursor to start of item in lane", filbCfg["moveEditCurToStartOfContent"])
	ImGui.SameLine(ctx)
	filbGUI.HelpMarker(
	...
	do
		...
		rv, filbSettings.curTimeSelectMode =
			ImGui.Combo(ctx, "Time selection", filbSettings.curTimeSelectMode, navTimeSelectMode)
	end

	-- PREVIEW
	ImGui.SeparatorText(ctx, "Preview")
	rv, filbCfg["previewOnLaneSelection"] =
		ImGui.Checkbox(ctx, "Preview on lane selection change", filbCfg["previewOnLaneSelection"])
	ImGui.SameLine(ctx)
	filbGUI.HelpMarker(
		'You can still preview the lane content with "Preview content under edit cursor in currently soloed lane" when toggled off.'
	)
	rv, filbCfg["previewMarkerName"] =
		ImGui.InputTextWithHint(ctx, "Preview marker name", filbCfg["previewMarkerName"], filbCfg["previewMarkerName"])
	ImGui.PushItemWidth(ctx, 130)
	do
		local markerLocation = "mouse cursor\0edit cursor\0"
		rv, filbSettings.curMarkerLocation = ImGui.Combo(
			ctx,
			"Target location for preview marker creation",
			filbSettings.curMarkerLocation,
			markerLocation
		)
	end
	ImGui.PushItemWidth(ctx, 130)
	do
		...
		rv, filbSettings.curMarkerLane =
			ImGui.Combo(ctx, "Target lane when creating marker at edit cursor", filbSettings.curMarkerLane, markerLane)
	end
	ImGui.SameLine(ctx)
	...
	do
		...
		rv, filbSettings.curSeekPlaybackRetriggCurPos = ImGui.Combo(
			ctx,
			"Cursor position when retriggered",
			filbSettings.curSeekPlaybackRetriggCurPos,
			seekPlaybackRetriggCurPos
		)
	end
	ImGui.PushItemWidth(ctx, 100)
	do
		local seekPlaybackEndCurPos = "last\0after last\0"
		rv, filbSettings.curSeekPlaybackEndCurPos = ImGui.Combo(
			ctx,
			"Cursor position when playback finishes",
			filbSettings.curSeekPlaybackEndCurPos,
			seekPlaybackEndCurPos
		)
	end
	ImGui.SameLine(ctx)
	filbGUI.HelpMarker(
	...
	rv, filbCfg["recallCursPosWhenRetriggRec"] = ImGui.Checkbox(
		ctx,
		"Recall cursor position when retriggering Record in place / with context",
		filbCfg["recallCursPosWhenRetriggRec"]
	)
	rv, filbCfg["scrollViewToEditCursorOnStopRecording"] =
		ImGui.Checkbox(ctx, "Scroll view to edit cursor on stop", filbCfg["scrollViewToEditCursorOnStopRecording"])
	rv, filbCfg["recordPunchInAtNextContentIfAny"] = ImGui.Checkbox(
		ctx,
	...
	rv, filbCfg["pushNextContentTime"] =
		ImGui.InputInt(ctx, '"Push next content when recording" amount in seconds', filbCfg["pushNextContentTime"])

	-- LANE PRIORITY
	ImGui.SeparatorText(ctx, "Lane Priority")
	rv, filbCfg["prioritizeCompLaneOverLastLane"] =
		ImGui.Checkbox(ctx, "Prioritize comp lanes over last lane", filbCfg["prioritizeCompLaneOverLastLane"])

	do
		local lanePriority = "first\0last\0"
		rv, filbSettings.curItemLaneCombo =
			ImGui.Combo(ctx, "Lane priority", filbSettings.curItemLaneCombo, lanePriority)
	end
	ImGui.SameLine(ctx, 0, 20)
	do
		local compPriority = "first\0last\0"
		rv, filbSettings.curItemCompCombo =
			ImGui.Combo(ctx, "Comp lane priority", filbSettings.curItemCompCombo, compPriority)
	end
	rv, filbCfg["exportTrackNameAppend"] = ImGui.InputTextWithHint(
		ctx,
		"Append export track name",
		filbCfg["exportTrackNameAppend"],
		filbCfg["exportTrackNameAppend"]
	)
	ImGui.SameLine(ctx, 0, 20)
	do
	...
	rv, filbCfg["makeRegionsForExport"] = ImGui.Checkbox(
		ctx,
		'Create regions with "Select all priority content... for render"',
		filbCfg["makeRegionsForExport"]
	)
	ImGui.SameLine(ctx)
	filbGUI.HelpMarker("Regions are auto-assigned to their export track in the region render matrix")
	rv, filbCfg["regionNameLeadingZero"] =
		ImGui.InputInt(ctx, "Add leading zeroes to region name", filbCfg["regionNameLeadingZero"])
	ImGui.SameLine(ctx)
	filbGUI.HelpMarker("With 2 leading zeroes:\nName7 becomes Name007\nName13 becomes Name013")
	rv, filbCfg["regionNameTrailingZero"] =
		ImGui.InputInt(ctx, "Add trailing zeroes to region name", filbCfg["regionNameTrailingZero"])
	ImGui.SameLine(ctx)
	...
	rv, filbCfg["showValidationErrorMsg"] = ImGui.Checkbox(
		ctx,
		"Display error messages when user selection is incorrect",
		filbCfg["showValidationErrorMsg"]
	)
