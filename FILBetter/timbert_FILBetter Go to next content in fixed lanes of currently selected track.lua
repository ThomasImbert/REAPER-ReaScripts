...
local vascoReady
vasco_functions = reaper.GetResourcePath() .. "/Scripts/TImbert Scripts/VASCO/timbert_VASCO functions.lua"
...
function main()
	...
	if reaper.CountTrackMediaItems(track) == 0 then
		if not vascoReady or (not vascoRegion.next and goToNextSnapToLastContent == false) then
			return
		elseif
			not vascoRegion.next
			and goToNextSnapToLastContent == true
			and (vascoRegion.current or vascoRegion.previous)
		then
	...
	if timbert.ValidateItemsUnderEditCursorOnSelectedTracks() == true then
		...
		if
			vascoRegion.next and reaper.GetCursorPosition() == startContent
			or reaper.GetCursorPosition() > vascoRegion.next.pos
		then
	...
	if timbert.ValidateItemsUnderEditCursorOnSelectedTracks() == false then
		...
		if
			select(2, reaper.get_action_context()) ~= debug.getinfo(1, "S").source:sub(2)
			or goToNextSnapToLastContent == false
		then
			reaper.SetMediaTrackInfo_Value(track, "C_ALLLANESPLAY", 0) -- DeActivate all lanes
			reaper.TrackList_AdjustWindows(true)
			GetTimeSelectionMode(goToContentTimeSelectionMode, startTime, endTime)
